...
<body>
	...
	<section class="main-section">
		...
		<h1>Reference <small>View</small></h1>
			...
			<c:if test="${ not empty id }">
				<c:if test="${ dto.id == id }"> <!-- 해당글의 글쓴 아이디와 로그인한 사람이 동일하다면.. -->
					<button type="button" class="btn btn-primary"
						onclick="location.href='/myapp/ref/edit.do?seq=${ dto.seq }';">수정하기</button>
					<button type="button" class="btn btn-primary"
						onclick="location.href='/myapp/ref/del.do?seq=${ dto.seq }';">삭제하기</button>
				</c:if>
			
				<button type="button" class="btn btn-primary"
					onclick="location.href='/myapp/ref/add.do?reply=1&thread=${ dto.thread }&depth=${ dto.depth }';">답변달기</button>

			</c:if>

			<button type="button" class="btn btn-default"
				onclick="location.href='/myapp/ref/list.do?column=${ column }&search=${ search }';">돌아가기</button>
		</div>
		...
		<div>
							...
							<c:if test="${ not empty id }">
								<c:if test="${ cdto.id == id }">
									<input type="button" value="삭제하기" class="btn btn-default" 
										onclick="location.href='/myapp/ref/delcomment.do?seq=${ cdto.seq }&pseq=${ dto.seq }';"/>
										<!-- 
			...
			<c:if test="${ not empty id }">
			<form method="POST" action="/myapp/ref/addcomment.do">
